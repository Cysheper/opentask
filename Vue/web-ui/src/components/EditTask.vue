<template>
  <div class="page">
    ...
    <form v-else-if="form" class="form-card" @submit.prevent="submit">
      <!-- 基本信息 -->
      ...
      <div class="form-footer">
        <router-link :to="'/task/' + id" class="btn btn-cancel">取消</router-link>
  ...
  methods: {
      ...
      try {
        ...
        if (data.status === 'ok') {
          ...
          for (var i = 0; i < data.tasks.length; i++) {
            if (data.tasks[i].id === Number(this.id)) {
              task = data.tasks[i]
              break
            }
          }
          if (task) {
            this.form = {
              ...
              code: task.code || '',
              immediately: !!task.immediately,
              target_count: task.target_count || 0,
              is_send: !!task.is_send,
              send_url: task.send_url || '',
              send_token: task.send_token || '',
              triggers: this.parseTriggers(task.trigger_time_list)
            }
    ...
    parseTriggers(list) {
      ...
      for (var i = 0; i < list.length; i++) {
        var tr = list[i]
        result.push({
          mouthStr: (tr.mouth && tr.mouth.length) ? tr.mouth.join(', ') : '',
          weekDayStr: (tr.week_day && tr.week_day.length) ? tr.week_day.join(', ') : '',
          mouthDayStr: (tr.mouth_day && tr.mouth_day.length) ? tr.mouth_day.join(', ') : '',
          hour: (tr.day_time && tr.day_time[0]) || 0,
          minute: (tr.day_time && tr.day_time[1]) || 0
        })
      }
      return result
    },
    updateHighlight() {
      var code = this.form && this.form.code ? this.form.code : ' '
      this.highlightedCode = hljs.highlight(code, { language: 'python' }).value
    },
    syncScroll() {
      var textarea = this.$refs.codeInput
      var pre = this.$refs.codeHighlight
      if (textarea && pre) {
    ...
    async submit() {
      ...
      try {
        var triggerList = []
        for (var i = 0; i < this.form.triggers.length; i++) {
          var t = this.form.triggers[i]
          triggerList.push({
        ...
        var payload = {
          ...
          trigger_time_list: triggerList
        }

        var res = await axios.post(API + '/update_task/' + this.id, payload)
        if (res.data.status === 'ok') {
          this.tip('修改已保存')
          var self = this
          setTimeout(function () { self.$router.push('/task/' + self.id) }, 800)
        } else {
          this.tip('保存失败: ' + res.data.message, 'err')
        }
      } catch (e) {
        void e
        this.tip('网络错误', 'err')
